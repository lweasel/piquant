...
#def _add_top_comment(writer,
        #read_depth = 10, read_length = 100, paired_end = False, errors = False,
        #stranded = False, bias = True, noise_perc = 0 ):
    #paired_end = "pe" if paired_end else "se"
    #errors = "errors" if errors else "no_errors"
    #stranded = "stranded" if stranded else "unstranded"
    #bias = "bias" if bias else "no_bias"
    #noise_perc = "noise-" + str(noise_perc) if noise_perc > 0 else "no_noise"
    #writer.add_comment(
            #"{read_d}x_{read_l}b_{paired}_{error}_{strand}_{biased}_{noise}".format(
                #read_d = read_depth, read_l = read_length, paired = paired_end,
                #error = errors, strand = stranded, biased = bias, noise = noise_perc
                #)
            #)
    #writer.add_comment("Read the abundance of all transctipts from the expression profile \
            #and run the polyester simulation using transcripts fasta file")


#def _add_simulation_params(writer,
        #read_depth = 10, read_length = 100, paired_end = False, errors = False,
        #stranded = False, bias = False, transcript_file = "polyester.transcripts.fa",
        #transcript_set = "main", out_dir = "polyester_tmp"
        #):
    #writer.add_comment("Set sequencing parameters and file names")
    #ps_params = _get_params(
            #read_depth, read_length, paired_end, errors,
            #stranded, bias, transcript_file,
            #transcript_set, out_dir
            #)
    #for names,values in ps_params.items():
        #script_line = names + " = " + str(values)
        #writer.add_line(script_line)
